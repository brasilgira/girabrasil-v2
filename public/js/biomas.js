...
(function () {
  ...
  function especieCard(item, tipo, idx) {
    ...
    return `
      <button class="especie-card" data-tipo="${tipo}" data-idx="${idx}">
        <div class="especie-imagem-wrap" data-especie-imagem="${item.nome.replace(/"/g, '&quot;')}" data-especie-cientifico="${item.cientifico.replace(/"/g, '&quot;')}" data-especie-tipo="${tipo}"${item.imagemUrl ? ` data-especie-imagem-url="${item.imagemUrl.replace(/"/g, '&quot;')}"` : ''}>
  ...
  // ----------------------------------------------------------------------
  // Fotos reais de fauna e flora (Wikipédia/Wikimedia)
  // ----------------------------------------------------------------------
  // Busca prioriza o NOME CIENTÍFICO, não o nome popular. Nomes populares
  // colidem com outras coisas (ex.: "Ariranha" também é um município de
  // São Paulo — buscar só por "Ariranha" trazia o mapa da cidade em vez do
  // animal). Nome científico é único no mundo todo, então evita isso quase
  // por completo.
  //
  // Além disso, o "pageimage" escolhido pela Wikipédia para a página certa
  // às vezes NÃO é uma foto — é um mapa de distribuição geográfica da
  // espécie (comum em artigos mais curtos). Esses mapas quase sempre são
  // arquivos .svg com "map"/"distribuição"/"distribution" no nome, então
  // esse tipo de resultado é filtrado e descartado antes de aparecer no
  // card — melhor cair no emoji de fallback do que mostrar um mapa.
  const cacheImagensEspecie = new Map();

  function pareceMapaOuNaoFoto(nomeArquivo, urlThumbnail) {
    const alvo = `${nomeArquivo || ''} ${urlThumbnail || ''}`.toLowerCase();
    if (/\.svg(\?|$)/i.test(urlThumbnail || '')) return true; // fotos reais nunca são .svg
    return /(mapa|map|distribui|distribution|localiza|range|bandeira|flag|brasao|coat_of_arms)/i.test(alvo);
  }

  async function buscarThumbnailPorTitulo(titulo) {
    const url = `https://pt.wikipedia.org/w/api.php?action=query&titles=${encodeURIComponent(titulo)}&prop=pageimages&piprop=thumbnail%7Cname&pithumbsize=700&redirects=1&format=json&origin=*`;
    ...
    if (!pagina || !pagina.thumbnail) return null;
    if (pareceMapaOuNaoFoto(pagina.pageimage, pagina.thumbnail.source)) return null;
    return pagina.thumbnail.source;
  }

  async function buscarThumbnailPorTermo(termo) {
    const url = `https://pt.wikipedia.org/w/api.php?action=query&generator=search&gsrsearch=${encodeURIComponent(termo)}&gsrnamespace=0&gsrlimit=1&prop=pageimages&piprop=thumbnail%7Cname&pithumbsize=700&format=json&origin=*`;
    const resposta = await fetch(url);
    if (!resposta.ok) return null;
    const dados = await resposta.json();
    const paginas = dados.query && dados.query.pages ? Object.values(dados.query.pages) : [];
  ...
  async function buscarImagemEspecie(nome, tipo, cientifico, imagemManual) {
    const chave = `${tipo}:${nome}`;
    if (cacheImagensEspecie.has(chave)) return cacheImagensEspecie.get(chave);

    // Substituto manual: quando a busca automática já se mostrou errada pra
    // essa espécie específica (ex.: a página da Wikipédia só tem um mapa de
    // distribuição, sem foto), o dado já vem com uma foto verificada fixa —
    // nesse caso nem tenta a busca automática, usa direto.
    if (imagemManual) {
      cacheImagensEspecie.set(chave, imagemManual);
      return imagemManual;
    }

    // Remove parênteses (ex.: sinônimos) do nome científico, se houver
    ...
    try {
      ...
      // 2) Sem artigo exato: busca pelo termo científico
      ...
      //    por isso só entra em jogo se as buscas mais precisas falharem)
      if (!imagem) {
  ...
  async function carregarImagensEspecies() {
    ...
    elementos.forEach(async (container) => {
      ...
      const imagemManual = container.getAttribute('data-especie-imagem-url');
      const imagem = await buscarImagemEspecie(nome, tipo, cientifico, imagemManual);
      if (!imagem) return;
  ...
  root.querySelectorAll('.especie-card').forEach(card => {
    card.addEventListener('click', async () => {
      ...
      const imagemModal = await buscarImagemEspecie(item.nome, tipo, item.cientifico, item.imagemUrl);
  ...
  // O objeto de áudio é criado uma única vez e reaproveitado nos cliques
  // seguintes, para que "pausar" realmente pause o que estava tocando em
  // vez de criar um novo áudio do zero a cada clique.
